...
static ImVec2 InputTextCalcTextSize(ImGuiContext *ctx, const char *text_begin, const char *text_end_display,
                                    const char *text_end, const char **out_remaining = NULL, ImVec2 *out_offset = NULL,
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect &bb, ImGuiComboFlags flags)
{
    ...
    if (!ret)
    {
        ...
        if (!g.IO.ConfigDebugBeginReturnValueOnce &&
            !g.IO.ConfigDebugBeginReturnValueLoop) // Begin may only return false with those debug tools activated.
                                    ...
                                    ImDrawTextFlags flags)
{
    ...
    return ImFontCalcTextSizeEx(g.Font, g.FontSize, FLT_MAX, obj->WrapWidth, text_begin, text_end_display, text_end,
                                out_remaining, out_offset, flags);
}
...
static void STB_TEXTEDIT_LAYOUTROW(StbTexteditRow *r, ImGuiInputTextState *obj, int line_start_idx)
{
    ...
    const ImVec2 size =
        InputTextCalcTextSize(obj->Ctx, text + line_start_idx, text + obj->TextLen, text + obj->TextLen,
                              &text_remaining, NULL, ImDrawTextFlags_StopOnNewLine | ImDrawTextFlags_WrapKeepBlanks);
    r->x0 = 0.0f;
    r->x1 = size.x;
...
static int STB_TEXTEDIT_MOVELINESTART_IMPL(ImGuiInputTextState *obj, ImStb::STB_TexteditState *state, int cursor)
{
    ...
    if (obj->WrapWidth > 0.0f)
    {
        ...
        while (p >= p_bol)
        {
            const char *p_eol = ImFontCalcWordWrapPositionEx(g.Font, g.FontSize, p, text_end, obj->WrapWidth,
                                                             ImDrawTextFlags_WrapKeepBlanks);
            if (p == p_cursor) // If we are already on a visible beginning-of-line, return real beginning-of-line (would
                               // be same as regular handler below)
...
static int STB_TEXTEDIT_MOVELINEEND_IMPL(ImGuiInputTextState *obj, ImStb::STB_TexteditState *state, int cursor)
{
    ...
    if (obj->WrapWidth > 0.0f)
    {
        ...
        while (p < text_end)
        {
            ...
            if (p_eol == p_cursor &&
                obj->LastMoveDirectionLR !=
                    ImGuiDir_Left) // If we are already on a visible end-of-line, switch to regular handle
                break;
            if (p_eol > p_cursor)
                return cursor;
            p = (*p_eol == '\n') ? p_eol + 1 : p_eol;
...
void ImGuiInputTextState::OnKeyPressed(int key)
{
    ...
    if (key_u == STB_TEXTEDIT_K_LEFT || key_u == STB_TEXTEDIT_K_LINESTART || key_u == STB_TEXTEDIT_K_TEXTSTART ||
        key_u == STB_TEXTEDIT_K_BACKSPACE || key_u == STB_TEXTEDIT_K_WORDLEFT)
        LastMoveDirectionLR = ImGuiDir_Left;
    else if (key_u == STB_TEXTEDIT_K_RIGHT || key_u == STB_TEXTEDIT_K_LINEEND || key_u == STB_TEXTEDIT_K_TEXTEND ||
             key_u == STB_TEXTEDIT_K_DELETE || key_u == STB_TEXTEDIT_K_WORDRIGHT)
...
// 'max_output_buffer_size' happens to be a meaningful optimization to avoid writing the full line_index when not
// necessarily needed (e.g. very large buffer, scrolled up, inactive)
static int InputTextLineIndexBuild(ImGuiInputTextFlags flags, ImGuiTextIndex *line_index, const char *buf,
                                   const char *buf_end, float wrap_width, int max_output_buffer_size,
...
static ImVec2 InputTextLineIndexGetPosOffset(ImGuiContext &g, ImGuiInputTextState *state, ImGuiTextIndex *line_index,
                        ...
                        ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void *callback_user_data)
{
    ...
    IM_ASSERT((flags & ImGuiInputTextFlags_WordWrap) == 0 ||
              (flags & ImGuiInputTextFlags_Password) == 0); // WordWrap does not work with Password mode.
    IM_ASSERT((flags & ImGuiInputTextFlags_WordWrap) == 0 ||
              (flags & ImGuiInputTextFlags_Multiline) != 0); // WordWrap does not work in single-line mode.
    ...
    if (is_multiline)
    {
        ...
        // Prevent NavActivation from explicit Tabbing when our widget accepts Tab inputs: this allows cycling through
        // widgets without stopping.
        if (g.NavActivateId == id && (g.NavActivateFlags & ImGuiActivateFlags_FromTabbing) &&
            !(g.NavActivateFlags & ImGuiActivateFlags_FromFocusApi) && (flags & ImGuiInputTextFlags_AllowTabInput))
            g.NavActivateId = 0;
    ...
    // Word-wrapping: enforcing a fixed width not altered by vertical scrollbar makes things easier, notably to track
    // cursor reliably and avoid one-frame glitches. Instead of using ImGuiWindowFlags_AlwaysVerticalScrollbar we
    // account for that space if the scrollbar is not visible.
    ...
    else
    {
        if (render_cursor || render_selection || g.ActiveId == id)
            ...
            buf_display_end = NULL; // Inactive multi-line: end of buffer will be output by InputTextLineIndexBuild()
                                    // special strchr() path.
        else
    ...
    if (is_multiline)
        line_count =
            InputTextLineIndexBuild(flags, line_index, buf_display, buf_display_end, wrap_width,
    ...
    const ImVec2 cursor_offset =
        render_cursor && state
            ? InputTextLineIndexGetPosOffset(g, state, line_index, buf_display, buf_display_end, state->Stb->cursor)
            : ImVec2(0.0f, 0.0f);
    ImVec2 draw_scroll;
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_selection)
        {
            ...
            const float bg_offy_up = is_multiline ? 0.0f : -1.0f; // FIXME: those offsets should be part of the style?
                                                                  // they don't play so well with multi-line selection.
            const float bg_offy_dn = is_multiline ? 0.0f : 2.0f;
            const float bg_eol_width =
                IM_TRUNC(g.FontBaked->GetCharAdvance((ImWchar)' ') * 0.50f); // So we can see selected empty lines
    ...
    if ((is_multiline || (buf_display_end - buf_display) < buf_display_max_length) && (text_col & IM_COL32_A_MASK) &&
        ...
        g.Font->RenderText(
            draw_window->DrawList, g.FontSize, draw_pos - draw_scroll + ImVec2(0.0f, line_visible_n0 * g.FontSize),
            text_col, clip_rect.AsVec4(), line_index->get_line_begin(buf_display, line_visible_n0),
            line_index->get_line_end(buf_display, line_visible_n1 - 1), wrap_width, ImDrawTextFlags_WrapKeepBlanks);
    ...
    if (render_cursor)
    {
        ...
        // Notify OS of text input position for advanced IME (-1 x offset so that Windows IME can cover our cursor. Bit
        // of an extra nicety.) This is required for some backends (SDL3) to start emitting character/text inputs. As
        // per #6341, make sure we don't set that on the deactivating frame.
...
static void ImGui::TabBarLayout(ImGuiTabBar *tab_bar)
{
    ...
    for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++)
    {
        ...
        section->WidthAfterShrinkMinWidth += ImMin(tab->ContentWidth, shrink_min_width) +
    ...
    const float width_all_tabs_to_use_for_scroll = (tab_bar->Flags & ImGuiTabBarFlags_FittingPolicyScroll)
    ...
    else if (tab_bar->ScrollButtonEnabled && IsMouseHoveringRect(tab_bar->BarRect.Min, tab_bar->BarRect.Max, true) &&
