...
void InterfaceMgr::DrawSidepanel()
{
    ...
    ImGuiWindowFlags flags = ImGuiWindowFlags_NoTitleBar + ImGuiWindowFlags_NoMove + ImGuiWindowFlags_NoCollapse + ImGuiWindowFlags_NoResize;
    ...
    if (ImGui::Begin("Info Menu", NULL, flags))
    {
        if (ImGui::BeginTabBar("Info Tab", ImGuiTabBarFlags_NoTooltip))
        {
                ...
                if (ImGui::BeginChild("Editor child"))
                {
                    ...
                    if (EntMgr.m_pSelected)
                    {
                        if (ImGui::CollapsingHeader("Object selection", ImGuiTreeNodeFlags_DefaultOpen))
                        {
                            ...
                            if (EntMgr.m_pSelected->m_nType == ENTITY_TYPE_OBJECT || EntMgr.m_pSelected->m_nType == ENTITY_TYPE_BUILDING)
                            {
                                static int bmodel = 0;
                            ...
                            CVector euler = data.GetEuler();
                            CQuaternion q = data.GetQuat();
                            if (ImGui::InputFloat3("Euler ##Obj", (float *)&euler))
                            {
                                euler.x = Utils::NormalizeAngle(euler.x);
                                euler.y = Utils::NormalizeAngle(euler.y);
                                euler.z = Utils::NormalizeAngle(euler.z);
                                data.SetEuler(euler);
                            }

                            if (ImGui::InputFloat4("Quat ##Obj", (float *)&q))
                            {
                                q.Normalise();
                                data.SetQuat(q);
                            }
            ...
            if (ImGui::BeginTabItem("Location"))
            {
                ...
                ImGui::Spacing();
                Widget::DataList(m_locData, [](std::string &root, std::string &key, std::string &loc)
                                 {
                    try {
                        int dimension = 0;
                        CVector pos;
                                    ...
                                    Viewport.m_Renderer.m_bShowNextFrame ? ImGuiTabItemFlags_SetSelected : NULL))
            {
                ...
                ImGui::Spacing();
                int model = Viewport.m_Renderer.GetSelected();
                ImGui::Text("Selected: %s, Model: %d", EntMgr.FindNameFromModel(model).c_str(), model);
